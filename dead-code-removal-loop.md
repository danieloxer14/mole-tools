...
- [x] `dead-code/redundant-fake-vcs-contract-tests.md` — 2026-08-30 — committed 3f872be — removed FakeVcs-only worktree contract tests and unused `forceRemoveWorktreeError`; retained GitAdapter/worktree-prune/review coverage; build/lint/CLI smoke passed; focused 90/0 and full 440/0 had no assertion failures but exited on pre-existing 90% coverage gate (64.31%/70.16%; 85.34%/88.89%).
...
- [x] `dead-code/review-agent-generic-parser.md` — 2026-08-30 — committed 0f27701 — removed orphaned generic parser/helpers and parser-only tests; retained OMP/Claude adapter normalization plus executor/port behavior coverage; build/lint/CLI smoke passed; focused 28/0 and full 436/0 had no assertion failures but exited on pre-existing 90% coverage gate (75.14%/87.82%; 85.12%/88.92%).
...
- [x] `dead-code/review-ui-unused-helper-exports.md` — 2026-08-30 — committed 0783368 — removed unused `actionLabel`/`stateLabel` named exports while retaining private helper calls and `ApprovalControls` rendering; temporary removal proof red/green; build/lint/CLI smoke passed; focused 9/0 and full 437/0 had no assertion failures but exited on pre-existing 90% coverage gate (24.13%/48.63% focused; 85.12%/88.92% full).
...
- [ ] `dead-code/review-ui-duplicate-column-constants.md` — Needs investigation — 2026-08-30 — no code changed; baseline blocked by enforced 90% coverage failures (focused UI 31.72% funcs / 53.77% lines; full 84.99% funcs / 88.36% lines), recorded in ticket.
