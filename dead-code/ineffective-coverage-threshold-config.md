...
- [x] Deleted temporary removal-proof test and reran retained `bun test --coverage --coverage-reporter=lcov` (441 / 0, `coverage/lcov.info`) and `bun test src/core/context.test.ts` (5 / 0 tests, expected exit 1 from threshold). Execution commit recorded in `dead-code-removal-loop.md`.
