...
describe("ralph run cost ledger", () => {
	...
	test("keeps worker edits when checklist is unchanged", async () => {
		dir = await mkdtemp(join(process.cwd(), ".ralph-run-"));
		setRalphDirForTesting(dir);
		await writeTaskFile("demo", TASK);
				...
				fakeContext({
					llm: new FakeLlm({
						agentResults: [
							{
								ok: true,
								output: "workspace work complete",
								usage: {
									inputTokens: 1,
									outputTokens: 1,
									cacheReadTokens: 0,
		...
		expect(
			ui.transcript.some((entry) =>
				String(entry.text).includes("Iteration failed"),
			),
		).toBe(false);
	});
	...
	test("persists workers and periodic/final reflections in order with summaries", async () => {
		...
		const iterationDebug = await readFile(
			join(dir, "demo.iteration-1.md"),
			"utf8",
		);
		expect(iterationDebug).toContain("# Ralph iteration 1");
		expect(iterationDebug).toContain("## Task file before");
		expect(iterationDebug).toContain("## Task file after");
		...
		expect(
			state.costLedger.every(
				(r) => r.provider === "pi" && r.model === "claude-sonnet-4",
			),
		).toBe(true);
		const aggregate = aggregateRalphCosts(state.costLedger);
		const finalRows = aggregate.rows.filter(
			(row) => row.label === "Final reflection",
		);
	...
	test("resumes with historical ledger entries without duplicating totals", async () => {
		...
		await runRalphRun(
			fakeContext({ llm: new FakeLlm(), ui: new FakeUiPort() }),
			{ name: "demo" },
		);
		const completed = await readState("demo");
		expect(completed.costLedger).toHaveLength(3);
		expect(aggregateRalphCosts(completed.costLedger).total).toEqual(
			totalBeforeSecondRun,
		);
	});
	...
	test("prints summary when already completed", async () => {
		...
		await writeState(
			"demo",
			stateFor({ status: "completed", phase: "completed" }),
		);
		const ui = new FakeUiPort();
		await runRalphRun(fakeContext({ llm: new FakeLlm(), ui }), {
			name: "demo",
		});
		expect(ui.transcript.some((e) => String(e.text).includes("Total"))).toBe(
			true,
		);
	});
	...
	test("prints summary on max-iteration pause", async () => {
		...
		expect(
			ui.transcript.filter((e) => String(e.text).includes("Total")).length,
		).toBeGreaterThan(0);
	});
