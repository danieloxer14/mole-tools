...
export const ralphRunArgs = z.object({
	name: LoopNameSchema,
...
export interface RalphRunResult {
	name: string;
	status: "ready_for_reflection" | "paused" | "completed";
	iteration: number;
}
export class RalphRunError extends Error {
	constructor(
		...
		super(message);
		this.name = "RalphRunError";
	}
}
function diagnostic(
	result: { stderr?: string; output?: string },
	fallback: string,
): string {
...
function reflectionRequest(taskFile: string): string {
	return [
		"Review the current Ralph loop implementation and task plan.",
		"Use the repository and verification evidence, then update the task file when necessary.",
		"You may uncheck inadequate tasks or add new unchecked tasks, but preserve all required headings and checklist meaning.",
		"\nRalph task file:\n",
		taskFile,
	].join("\n");
}
function assertTaskPath(name: string, state: RalphStateFile): void {
	if (state.taskFile !== `.ralph/${name}.md`)
		throw new Error(`State task path mismatch: expected .ralph/${name}.md`);
}

export async function runRalphRun(
	ctx: Context,
	input: RalphRunArgs,
	...
	if (args.maxIterations !== undefined) {
		if (args.maxIterations <= state.maxIterations)
			throw new Error(
				`--maxIterations must be greater than the persisted cap (${state.maxIterations})`,
			);
	...
	if (
		!ctx
			.getLlmFor("ralph", state.models.reflect.provider)
			.capabilities()
			.includes("agentic-workspace")
	)
		throw new UnsupportedCapabilityError(
			"agentic-workspace",
	...
	const lock: LockHandle = createLock(args.name, runId);
	...
	const reflect = async (
		finalReview: boolean,
	): Promise<"reopened" | "complete"> => {
		const before = await readTaskFile(args.name);
		if (before === null)
			throw new Error(`Task file disappeared for "${args.name}"`);
		snapshotTaskFile(args.name);
		state = {
			...state,
			phase: PhaseEnum.reflecting,
			awaitingReview: finalReview,
			...(finalReview ? { status: StatusEnum.completed } : {}),
		};
		await writeState(args.name, state);
		...
		if (!result.ok || after === null || checked instanceof RalphParseError) {
			const error = !result.ok
				? diagnostic(result, "Reflection failed")
				: checked instanceof RalphParseError
					? checked.message
		...
		state = {
			...state,
			lastReflectionAt: state.iteration,
	...
	try {
		...
		while (true) {
			...
			try {
				result = await ctx.llm.runAgent({
					purpose: "ralph",
					providerKey: state.models.implement.provider,
			...
			if (state.reflectEvery > 0 && state.iteration % state.reflectEvery === 0)
				await reflect(false);
		}
	} finally {
