...
describe("estimateClaudeCosts", () => {
	...
	test("prices cache reads at 0.1x and cache writes at 1.25x the input rate", () => {
		...
		expect(estimates.find((e) => e.model === "Haiku 4.5")?.cost).toBeCloseTo(
			4.45,
		);
		expect(estimates.find((e) => e.model === "Sonnet 5")?.cost).toBeCloseTo(
			13.35,
		);
		expect(estimates.find((e) => e.model === "Opus 4.8")?.cost).toBeCloseTo(
			22.25,
		);
	});
