...
export function parseCodeowners(text: string): string[] {
	...
	for (const line of text.split(/\r?\n/)) {
		...
		for (const token of tokens) {
			if (token.startsWith("@") && token.length > 1) handles.push(token.slice(1));
		}
	}
	return [...new Set(handles)];
}

export async function findCodeowners(root: string): Promise<string | null> {
	...
	try {
		for await (const relative of glob.scan({ cwd: root, onlyFiles: true })) {
			...
			if (normalized.split("/").length <= CODEOWNERS_DEPTH &&
				!normalized.startsWith(".git/") && !normalized.startsWith("node_modules/"))
				return `${root}/${normalized}`;
		}
	} catch {
		// A repository root may disappear between git discovery and this optional
		// enrichment step; missing CODEOWNERS is a normal bypass, not a failure.
	}
	return null;
}
...
export function matchAuthorToMember(author: string, members: HostMember[]): HostMember | null {
	...
	const rules = [
		(member: HostMember) => normalized(member.handle) === name || normalized(member.displayName ?? "") === name,
		(member: HostMember) => first !== "" && last !== "" && normalized(member.handle).startsWith(first[0]) && normalized(member.handle).includes(last),
		(member: HostMember) => first !== "" && last !== "" && normalized(member.handle).startsWith(last[0]) && normalized(member.handle).includes(first),
		(member: HostMember) => normalized(member.handle).startsWith(name) || name.startsWith(normalized(member.handle)),
	];
...
export function rankReviewerSuggestions(
	...
	const available = members.filter((member) =>
		!currentUser || (member.id !== currentUser.id && normalized(member.handle) !== normalized(currentUser.handle)),
	);
	const byHandle = new Map(available.map((member) => [normalized(member.handle), member]));
	const picked = new Map<string, ReviewerSuggestion>();
	const add = (member: HostMember, commits: number, source: ReviewerSuggestion["source"]) => {
...
export async function selectReviewers(ctx: Context, base: string): Promise<string[]> {
	if (!ctx.gitHost) return [];
	const path = await findCodeowners(await ctx.vcs.repoRoot());
	if (!path) return [];
	const handles = parseCodeowners(await Bun.file(path).text());
	const members = (await Promise.all(handles.map((handle) => ctx.gitHost!.resolveHandle(handle))))
	...
	const suggestions = rankReviewerSuggestions(
		members,
	...
	if (await ctx.ui.confirm("Add manual reviewer handles?")) {
		const manual = await ctx.ui.editText("Reviewer handles", "");
		selected.push(...manual.split(/[\s,]+/).map((value) => value.replace(/^@/, "")).filter(Boolean));
	}
