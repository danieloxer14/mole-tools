...
- [ ] `mole-tools init` writes Ollama and Pi connection defaults plus the commit and merge-request routes required by the source spec.
- [ ] Commit and merge-request calls use their configured provider and model name with no hardcoded model fallback.
- [ ] README and ADR configuration guidance describe the explicit model shape and strict provider validation.
...
- Provider adapters are composed by `buildAdapterMap`; preserve the provider-neutral `Llm` port.
...
Interactive UI changes and feature behavior beyond route selection.
