...
- **Needs investigation (resolved):** 2026-08-28 — prior baseline was blocked by one unrelated flaky review-route test; rerun `bun run build && bun run lint && bun test` is green with 441 passing, 0 failing. Removal may proceed.
...
- [x] Captured required baseline with `bun run build && bun run lint && bun test`: build passed, lint passed (`Checked 145 files`), and full test passed (`441 pass`, `0 fail`, `1075 expect()` calls across 59 files).
- [x] Added temporary `test/dead-code/documentation-broken-cross-links.test.ts`; `bun test test/dead-code/documentation-broken-cross-links.test.ts` was RED before repairs (`0 pass`, `2 fail`) and GREEN after repairs (`2 pass`, `0 fail`, `16 expect()` calls).
- [x] Corrected only Markdown navigation paths in `specs/architecture/architecture.md`, `specs/architecture/implementation-plan.md`, `specs/architecture/code-design.md`, `specs/help-feature/help-feature.md`, `specs/merge-request/merge-request-tool.md`, and `specs/merge-request/merge-request-implementation-plan.md`; no historical wording or runtime/user-repository path changed.
- [x] Corrected generated `Source spec` paths only in `specs/help-feature/help-feature-implementation-plan.md` and `specs/explicit-model-routing/tickets/README.md`; preserved historical wording/status and did not alter imports, registrations, config/schema, fixtures, runtime artifacts, release scripts, or user-repository discovery paths.
- [x] Post-repair verification passed: scoped Python `glob`/`re` Markdown scan covered 49 files with `unresolved=0`; all seven `test -f` target checks passed; temporary proof test passed (`2 pass`, `0 fail`).
- [x] Deleted temporary proof test; final `bun run build && bun run lint && bun test` passed (build passed, lint checked 145 files, `441 pass`, `0 fail`, `1075 expect()` calls across 59 files). No CLI/UI/release smoke was required for static Markdown.
- [x] Final compatibility checks passed: obsolete broken link variants absent from affected docs, corrected targets present, no `Source spec`/companion-path consumer in `src/`, `scripts/`, or `test/`, no dynamic/registration/config/API/persisted-state/release/installer consumer, and historical/archival wording preserved.
