...
### Ralph state file
The machine-readable state at `.ralph/<name>.state.json` that persists a Ralph loop’s iteration count, lifecycle status, settings, selected provider/model, worker metadata, completion information, and cumulative Ralph cost data. Its required cost ledger is introduced as a breaking state-schema change; old state files without it are out of scope. Ralph prints its persisted cost summary after successful initialization and after every terminal run outcome, including completion, pauses, failures, and interruption. A run summary lists the full loop’s iteration-cost history, not only the invocation’s new iterations. mole-tools, rather than an agent worker, owns all of its updates. It retains runtime diagnostics: an orchestrator-generated worker run ID, the current/most recent worker item, timestamps, the most recent error, and the latest iteration handoff summary. Its `phase` supplies live UI detail and is one of `ready`, `implementing`, `reflecting`, `paused`, or `completed`; `status` remains the authoritative lifecycle field. A newly initialized state is `ready` at iteration zero; it becomes `in_progress` while continuing implementation (including after a failed attempt, which records its error and consumes an iteration), `paused` with reason `max_iterations_reached` when its cap is reached, and `completed` only after the completion gate and final review pass. Its provider is selected from the Ralph feature profile during initialization; its required model is supplied by the CLI. Both are persisted and reused by later worker and reflection sessions. Worker and timestamp fields begin only when `ralph run` executes. The “state iteration file” refers to this file; it is distinct from the Markdown task file.

### Ralph iteration handoff summary
A concise worker-produced account of work completed, verification performed, blockers, and next focus for one worker iteration. The worker returns it in a tagged `RALPH_ITERATION_SUMMARY` / `END_RALPH_ITERATION_SUMMARY` block. Reflections do not produce or replace this summary. Missing or malformed summary does not discard valid worker changes; Ralph persists an empty summary for that iteration. Ralph recommends a 2,000-character summary limit to workers and trims longer responses before persistence; this is a prompt/output policy, not a schema validation constraint. Ralph persists the latest summary in the state file and supplies it to the next worker session, including after resume. The first worker receives an explicit empty handoff section; later workers receive the latest summary as advisory quick-reference context for bootstrapping, while the task file and repository remain authoritative.
